...
async def build_referrals_report(db: AsyncSession) -> ReferralsReport:
    """
    Build a referrals analytics report aggregating totals, period counts, trends, distributions, and top referrers.

    This function collects referral reward metrics including totals, status breakdowns,
    time-period aggregations, trend analysis, growth rates, and top performers.

    Args:
        db (AsyncSession): Database session used to fetch referral metrics.

    Returns:
        ReferralsReport: Pydantic data structure containing aggregated referral analytics.
